...
def parse_report(text: str) -> dict | None:
    ...
    # Кол-во вышедших на работу (число после ✅)
    ...
    # Карты по типам
    cards = {}
    for card_type in ["Debit", "Credit", "Repacking card", "Treaty", "Visa", "Master card"]:
        pattern = rf"{re.escape(card_type)}[:\-\s]+(\d+)"
...
def get_monthly_stats(data: dict) -> dict:
    ...
    team_totals = [v["total_team"] for v in records.values() if v.get("total_team")]
    avg_team_total = round(sum(team_totals) / len(team_totals)) if team_totals else 0
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if parsed is None:
        await update.message.reply_text(
            ...
            "Убедись что пересылаешь правильный отчёт из группы.\n\n"
            f"Ищу: *{MY_NAME}*",
            parse_mode="Markdown"
        )
        return
    ...
    if day_key in data["records"]:
        await update.message.reply_text(
            f"⚠️ Запись за *{day_key}* уже есть. Обновляю...",
            parse_mode="Markdown"
        )
...
async def show_help(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "❓ *Как пользоваться:*\n\n"
        "1️⃣ Зайди в группу с ежедневными отчётами\n"
        "2️⃣ Нажми и удержи сообщение → *Переслать*\n"
        "3️⃣ Перешли мне\n"
        "4️⃣ Я найду твою строку и посчитаю зарплату\n\n"
        "📊 *Мой KPI за месяц* — общая статистика и сумма\n"
        "📅 *История* — все дни с зарплатой\n\n"
        f"🔍 Ищу имя: *{MY_NAME}*\n"
...
app.add_handler(MessageHandler(filters.FORWARDED, handle_text))
    print("🤖 Бот запущен...")
    app.run_polling()
